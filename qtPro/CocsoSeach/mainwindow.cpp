...
void MainWindow::on_calibrateButton_toggled(bool checked)
{
    if(checked){
        ...
        dbImgsList = export_folder.entryList();
        for ( QStringList::Iterator it = dbImgsList.begin(); it != dbImgsList.end(); ++it )
            qDebug() << "Processed command: " << *it;
        QStringListModel *model = new QStringListModel(this);
        // Populate our model
        model->setStringList(dbImgsList);
...
{
    QString imgName =  dir+'/'+dbImgsList.at(index.row());
    qDebug() << imgName;
    QPixmap img(imgName);
